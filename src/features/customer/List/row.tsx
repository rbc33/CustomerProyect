...
const Row = ({ item }: RowProps) => {
  ...
  return (
    <SwipeRow
      ...
      {renderHiddenItem()}
      <TouchableOpacity onLongPress={() => navigate('Edit Customer', { customerID: item.id })}>
        <View
          key={item.id}
          style={{
            borderWidth: 1,
          ...
          <Text key="ln">Active?: {item.active ? 'active' : 'inactive'}</Text>
          <Text key="ln">Region: {item.region}</Text>
        </View>
      </TouchableOpacity>
    </SwipeRow>
  );
